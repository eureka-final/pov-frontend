...
import { getReviews, getMyReviews, getClubReviews, getDetailReview, getJoinClub, getMovieReviews } from '../../apis/review/getReviews';
...
export const useMovieReviewsQuery = (movieId: string) => {
  const {
    ...
    fetchNextPage,
    isFetchingNextPage
  } = useInfiniteQuery<MovieReviewsResponse, Error>({
    queryKey: ['movieReviews', movieId],
    queryFn: async ({ pageParam = 0 }) => {
      const response = await getMovieReviews(movieId, pageParam);
      // 응답 데이터 검증
      if (!response || !response.data || !response.data.reviews) {
        throw new Error('Invalid API response structure');
      }
