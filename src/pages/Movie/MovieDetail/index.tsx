...
import { Heading, Body, Icon, ShowMorebtn } from 'pov-design-system';
import {
  Container,
  HeaderContainer,
  Additionals,
...
const Index = () => {
  ...
  return (
    <Basic>
      {movieData && (
        <Container>
          <HeaderContainer src={movieData.data.backdrop}>
            <BackgroundLayer src={movieData.data.backdrop}></BackgroundLayer>
            <HeaderInfo>
              <Heading size="large">{movieData.data.title}</Heading>
              <BodyContainer>
                <Body size="large">{movieData.data.released}</Body>
                <Body size="large">{movieData.data.directors}</Body>
